...
export function enforceSize(text, label) {
...
export async function collectSnapshot(repo, baseRef = 'origin/main', headRef = 'HEAD') {
    ...
    if (files.some(sensitivePath))
        throw new Error('The diff touches a likely credential file. Remove secrets from the comparison before evaluating.');
    ...
    if (!headPaths.includes('CLAUDE.md') && !baseRules)
        throw new Error('Missing committed CLAUDE.md project rules.');
    if (!headPaths.includes('.context/active-task.md'))
        throw new Error('Missing committed .context/active-task.md.');
    const readBlob = async (commit, path) => {
        const entry = await git(repo, ['ls-tree', commit, '--', path]);
        if (!/^100(644|755) blob /.test(entry))
            throw new Error('Context must contain regular Markdown files, not links or submodules.');
    ...
    if (!rules.trim())
        throw new Error('CLAUDE.md must contain project rules.');
    const context = {};
    ...
    if (!context['.context/active-task.md']?.trim())
        throw new Error('The canonical active task must not be empty.');
    const warnings = [];
    if (!baseRules)
        warnings.push('Bootstrap audit: base has no CLAUDE.md; rules come from the evaluated head and are not independently trusted.');
    if ((await git(repo, ['status', '--porcelain'])).trim())
        warnings.push('Working-tree changes are excluded; only committed changes and context are evaluated.');
    if (/^Binary files .* differ$/m.test(diff))
        warnings.push('Binary changes are listed, but their contents are not evaluated.');
