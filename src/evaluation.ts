...
export function object(value: unknown): Record<string, unknown> {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('Invalid model response: expected an object.');
...
function text(value: unknown): string {
  if (typeof value !== 'string' || !value.trim() || value.length > 30_000) throw new Error('Invalid model response: empty or oversized text.');
  return value;
}
function list(value: unknown): unknown[] {
  if (!Array.isArray(value) || value.length > 100) throw new Error('Invalid model response: expected a bounded list.');
  return value;
}
...
export function validateEvaluation(value: unknown): Evaluation {
  ...
  if (!['pass', 'warn', 'fail'].includes(String(audit.status))) throw new Error('Invalid audit status.');
  const findings = list(audit.findings).map((item): Evaluation['audit']['findings'][number] => {
    const finding = object(item);
    keys(finding, ['rule', 'severity', 'explanation', 'evidence']);
    if (finding.severity !== 'warning' && finding.severity !== 'error') throw new Error('Invalid finding severity.');
