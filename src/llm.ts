...
import type { Evaluation, Provider, Snapshot } from './types.js';
...
export async function evaluate(snapshot: Snapshot, options: LlmOptions): Promise<Evaluation> {
  if (!options.apiKey.trim()) throw new Error(`Missing ${options.provider === 'anthropic' ? 'ANTHROPIC_API_KEY' : 'OPENAI_API_KEY'}.`);
  if (!options.model.trim()) throw new Error('Set a model with --model or KEYSTONE_MODEL.');
  ...
  if (input.includes(options.apiKey)) throw new Error('Repository input contains the active API credential; nothing was sent.');
  ...
  let value: unknown;
  if (anthropic) {
    if (data.stop_reason !== 'tool_use' || !Array.isArray(data.content)) throw new Error('Provider returned an incomplete or refused evaluation.');
    const blocks = data.content.map(object).filter(block => block.type === 'tool_use' && block.name === 'record_evaluation');
    if (blocks.length !== 1) throw new Error('Provider did not return exactly one evaluation.');
    value = blocks[0]!.input;
  } else {
    if (data.status !== 'completed' || !Array.isArray(data.output)) throw new Error('Provider returned an incomplete or refused evaluation.');
    ...
    if (blocks.some(block => block.type === 'refusal')) throw new Error('Provider refused the evaluation.');
    const content = blocks.filter(block => block.type === 'output_text');
    if (content.length !== 1 || typeof content[0]!.text !== 'string') throw new Error('Provider did not return exactly one evaluation.');
    try { value = JSON.parse(content[0]!.text); } catch { throw new Error('Provider returned invalid JSON.'); }
  }
  const evaluation = validateEvaluation(value);
  if (JSON.stringify(evaluation).includes(options.apiKey)) throw new Error('Provider response contained a credential and was discarded.');
  return evaluation;
}
